...
class WalkInVisitors extends Component
{
        ...
        if ($this->search) {
            return $query->where(function($q) {
                $q->where('visitors.first_name', 'like', '%' . $this->search . '%')
                  ...
                  ->orWhere('visitors.check_in', 'like', '%' . $this->search . '%')
                  ->orWhere('visitors.check_out', 'like', '%' . $this->search . '%')
                  ->orWhere('visitors.national_id_no', 'like', '%' . $this->search . '%')
                  ->orWhere('visitors.gender', 'like', '%' . $this->search . '%')
                    // For date searching
    ...
    public function render()
    {
        ...
        return view('livewire.walk-in-visitors', ['visitors' => $visitors]);
    }
